...
const metaMaskConnect = <Element>(
  document.querySelector("#metamask-connect-btn")
);
...
const disconnectWallet = <Element>(
  document.querySelector("#wallet-disconnect-btn")
);
...
const computeSalaryBtn = <Element>(
  document.querySelector("#compute-average-salary-btn")
);
...
function contractAddressClick() {
  ...
  if (address === undefined) {
    ...
    const currentAddress = <HTMLInputElement>(
      document.querySelector("#current-address")
    );
    currentAddress.innerHTML = `Average Salary Address: ${address}`;
    const browserLink = <HTMLInputElement>(
      document.querySelector("#browser-link")
    );
...
function computeAction() {
  ...
  const api = getAverageApi();
  const browserLink = <HTMLInputElement>(
    document.querySelector("#compute-transaction-link")
  );
  browserLink.innerHTML = '<br><div class="loader"></div>';
